import { useState, useEffect, useCallback } from "react";
import { getSalesByPeriod } from "../services/api";
import SalesTrendChart from "../components/charts/SalesTrendChart";

export default function SalesTrendPage() {
  const [salesData, setSalesData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [params, setParams] = useState({
    from: "",
    to: "",
    group: "day",
  });

  const fetchData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const query = {
        from: params.from || undefined,
        to: params.to || undefined,
        group: params.group || "day",
      };
      const data = await getSalesByPeriod(query);
      if (data.success) setSalesData(data);
    } catch (err) {
      setError(
        err?.response?.data?.message ||
          err.message ||
          "Failed to fetch sales trend"
      );
    } finally {
      setLoading(false);
    }
  }, [params]);

  useEffect(() => {
    fetchData();
  }, [fetchData]);

  // Reset everything to initial state on retry
  const handleRetry = useCallback(() => {
    setParams({ from: "", to: "", group: "day" });
    setSalesData(null);
    setError(null);
  }, []);

  if (loading) {
    return (
      <div style={{ padding: "2rem", textAlign: "center" }}>
        Loading sales trend...
      </div>
    );
  }

  if (error) {
    return (
      <div style={{ padding: "2rem" }}>
        <div style={{ color: "#ef4444", marginBottom: "1rem" }}>
          Error: {error}
        </div>
        <button
          onClick={handleRetry}
          style={{
            padding: "0.5rem 1rem",
            backgroundColor: "#3b82f6",
            color: "white",
            border: "none",
            borderRadius: "0.375rem",
            cursor: "pointer",
          }}
        >
          Retry
        </button>
      </div>
    );
  }

  return (
    <div>
      <div
        style={{
          marginBottom: "1rem",
          padding: "0.75rem",
          backgroundColor: "#f9fafb",
          borderRadius: "8px",
        }}
      >
        <label
          style={{
            display: "block",
            fontSize: "0.85rem",
            marginBottom: "0.5rem",
            fontWeight: 600,
          }}
        >
          Filter Options
        </label>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          <input
            type="date"
            value={params.from}
            onChange={(e) => setParams((p) => ({ ...p, from: e.target.value }))}
            style={{ padding: "0.4rem", minWidth: "140px" }}
          />
          <input
            type="date"
            value={params.to}
            onChange={(e) => setParams((p) => ({ ...p, to: e.target.value }))}
            style={{ padding: "0.4rem", minWidth: "140px" }}
          />
          <select
            value={params.group}
            onChange={(e) =>
              setParams((p) => ({ ...p, group: e.target.value }))
            }
            style={{ padding: "0.4rem", minWidth: "140px" }}
          >
            <option value="day">Day</option>
            <option value="week">Week</option>
            <option value="month">Month</option>
          </select>
        </div>
      </div>
      <SalesTrendChart data={salesData} title="Sales Trend" height={400} />
    </div>
  );
}
